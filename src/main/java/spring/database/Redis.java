...
public class Redis {
    ...
    public void getConnection() {
        InputStream input = Redis.class.getClassLoader().getResourceAsStream("config");
    ...
    public ArrayList<Integer> getRegNoByColor(String color) {
        ArrayList<Integer> registrationNo=new ArrayList<>();
        for (int i = 1; i < count; i++) {
            Map<String, String> car = jedis.hgetAll(keyName + i);
            if(car.isEmpty())
                continue;
